...
#' @examples
#' file.dir <- "~/Dropbox/Research/BIGtrade/data/raw_data/sitcv1-244-decades/dynamic_2012.csv"
#' df <- computeDsWs(file.dir)
#' save(df, file = "~/Dropbox/Research/BIGtrade-select/data/likelihood_test/D_ijtk-W_ijtk.Rdata")
computeDsWs <- function(file.dir = NULL){
  ...
  # read sample data
  sampleData <- read.table(file.dir, 
                           header = TRUE, 
                           check.names = FALSE, nrow = 1)
  ncols <- ncol(sampleData)
  
  # read in all data
  df.r <- read.table(file.dir,
                     header = TRUE,
                     check.names = FALSE,
  ...
  df.l <- df.r %>%
    ...
    arrange(year, importer_ISO, exporter_ISO)
  ...
  df.out <- df.l%>%
    group_by(importer_ISO, exporter_ISO) %>%
    mutate(tot_direct_dyad_year_volume = sum(X_ijtk, na.rm = TRUE),
  ...
  # create vars
  df.out <- df.out %>% 
    select(importer_ISO, exporter_ISO, sitc, D_ijtk, W_ijtk) %>%
    mutate(dyad_id = ifelse(as.numeric(importer_ISO) < as.numeric(exporter_ISO), 
